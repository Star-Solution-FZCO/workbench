...
class AuthorizationServer(Generic[TRequest, TStorage]):
    ...
    def is_secure_transport(self, request: TRequest) -> bool:
        ...
        if request.settings.INSECURE_TRANSPORT:
            return True
        return bool(request.url.lower().startswith("https://") or request.headers.get('X-Forwarded-Proto') == 'https')
    ...
    async def create_token_introspection_response(self, request: TRequest) -> Response:
        ...
        if token_type == "access_token":  # nosec hardcoded_password_string
    ...
    async def revoke_token(self, request: TRequest) -> Response:
        ...
        access_token = (
            request.post.token
            if request.post.token_type_hint != "refresh_token"  # nosec hardcoded_password_string
            else None
        )
        refresh_token = (
            request.post.token
            if request.post.token_type_hint != "access_token"  # nosec hardcoded_password_string
            else None
        )
